  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#f2ebdc]">
      ...
      <main className="flex-1 overflow-y-auto p-6">
        <h1 className="text-center text-3xl font-bold text-teal-800 mb-6">Inicio</h1>
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="overflow-y-auto max-h-[300px] pr-2">
            {encuestasFiltradas.map((e, i) => (
              <EncuestaCard key={i} titulo={e.titulo} respuestas={e.respuestas} />
            ))}
          </div>
        </div>

        <CreateButton onClick={() => alert("Crear nueva encuesta")} />
      </main>
    </div>
  )
}
